...
- `target_key` 使用 webhook URL 的稳定哈希，继续支撑去重和历史展示。
- 首版采用最小闭环实现，后续可在 `personal_feishu` driver 内继续扩展更复杂的 webhook 消息格式。

## 决策 3F：Personal Feishu webhook 保持 text 消息，Markdown 只保留文本可读性

### 标题

`personal_feishu` 的 webhook 首版保持 `msg_type=text`，不复用企业版 `post + md` 富文本结构。

### 背景

用户在真实 webhook 验证中发现 `personal_feishu` 发送失败，说明当前 Personal Feishu 目标并不支持企业版 `post + md` 结构。回查 `docs/feishu-message-content.md` 后，文档明确声明整篇内容结构文档“不适用于自定义机器人”，且文本样式、超链接等能力也标注为“不支持自定义机器人”。

### 备选方案

- 方案 A：保持 webhook `text`，只保留换行和纯文本可读性。
- 方案 B：使用 webhook `post`，并把 Markdown 直接放入 `md` 节点。
- 方案 C：手工把 Markdown 拆成 `text` / `a` / `at` / `code_block` 等细粒度节点。
...
- 与当前文档边界一致，不再把企业版消息能力错误套用到自定义机器人。
- 实际 webhook 已验证失败，优先保证渠道稳定可用。
- 当前仓库没有自定义机器人专用富文本协议文档，强行模拟 `post` 或细粒度节点风险更高。

### 关键假设

- Personal Feishu 当前目标 webhook 至少支持 `msg_type=text`。
- Markdown 在 Personal Feishu 渠道只能保证“文本可读性”，不能保证样式级渲染。

### 影响

- `personal_feishu` 保持 `feishu_text` 渲染模式。
- 标题和正文会合并为文本消息发送，保留换行，但不承诺富文本样式生效。
- 若后续需要支持自定义机器人卡片或其专用 Markdown 协议，需要基于自定义机器人专属文档单独实现，而不是复用 Enterprise Feishu 的 `post` 协议。
